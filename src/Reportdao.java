...
public class Reportdao extends JFrame {
    ...
    public Reportdao(String callNumber, String reportDate) {
        ...
        try {
            ...
            PreparedStatement ps = con.prepareStatement("SELECT books.callno, books.name, books.quantity, books.issued, COUNT(issuebooks.studentid) AS DueBooks FROM books, issuebooks WHERE books.callno = ? AND issuebooks.bookcallno = ? AND DATEDIFF(?, issuebooks.issueddate) >= '15'", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
